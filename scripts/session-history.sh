#!/usr/bin/env bash
# ~/.config/tmux/scripts/session-history.sh
#
# Two session-navigation primitives over a single duplicate-free timeline:
#
#   1. trailing toggle  — flip to the session you were just in (Alt-Tab style).
#      Self-heals: if that session was deleted, falls back to the nearest live
#      neighbor instead of breaking like `switch-client -l`.
#   2. history stack    — browser-style back/forward. Strict browser semantics:
#      back/forward/toggle WALK the timeline; any other switch (session picker,
#      sessionx, manual) is a NAVIGATION that collapses forward history and
#      becomes the end of the road — pressing forward from there does nothing.
#
# MODEL:
#   @session_hist     ordered timeline of visited sessions, NO DUPLICATES
#   @session_idx      cursor position (index of the current session)
#   @session_current  last-known current session (so the hook can diff from→to)
#   @session_prev     the session the toggle flips to
#   @session_walk     transient flag: a walk (back/forward/toggle) names its
#                     target here so the hook knows NOT to collapse history
#
# How the hook tells a walk from a navigation:
#   • back/forward/toggle set @session_walk = <target> an instant before they
...
load() {
    CURRENT="$(G @session_current)"
    IDX="$(G @session_idx)"; [ -z "$IDX" ] && IDX=0
    PREV="$(G @session_prev)"
    local raw; raw="$(G @session_hist)"
    HIST=()
    [ -n "$raw" ] || return
    local seen=""
...
save() {
    S @session_current "$CURRENT"
    S @session_idx "$IDX"
    S @session_prev "$PREV"
    if [ "${#HIST[@]}" -eq 0 ]; then S @session_hist ""
    else local IFS=$'\n'; S @session_hist "${HIST[*]}"; fi
}
...
do_hook() {
    ...
    if [ -z "$CURRENT" ]; then                       # first fire / after reset
        HIST=("$to"); IDX=0; CURRENT="$to"; PREV="$to"; S @session_walk ""; save; return
    fi
    [ "$to" = "$CURRENT" ] && { S @session_walk ""; return; }
    ...
    CURRENT="$to"
    S @session_walk ""                                 # always clear the flag
    save
}
...
do_toggle() {
    ...
    # No @session_walk flag: the hook treats this as a navigation -> the target
    # is deduped and appended at the tip, so afterward the user is at the end of
    # the road and can only go backward.
...
do_back() {
    ...
    for (( i=IDX-1; i>=0; i-- )); do
        if session_exists "${HIST[$i]}" && [ "${HIST[$i]}" != "$CURRENT" ]; then
            S @session_walk "${HIST[$i]}"; tmux switch-client -t "${HIST[$i]}"; return
        fi
    done
...
do_pick() {
    load; reconcile "${1:-}"
    local cur="$CURRENT" items="" i s sel
    ...
    # NOTE: no @session_walk flag -> the hook treats this as a NAVIGATION,
...
do_reset() {
    S @session_hist ""; S @session_idx 0
    S @session_current ""; S @session_prev ""; S @session_walk ""
}
