...
# Non-active pane border color and settings (always applies to non-active panes)
...
# Active pane border settings for normal state (no prefix/copy mode)
ACTIVE_BORDER_LINES=$(tmux show-option -gv @active_border_lines 2>/dev/null)
ACTIVE_BORDER_LINES=${ACTIVE_BORDER_LINES:-heavy}
...
if [ "$CLIENT_PREFIX" = "1" ]; then
    ...
    ACTIVE_PANE_BORDER_LINES="$ACTIVE_BORDER_LINES"
